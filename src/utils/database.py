...
class DatabaseManager:
    ...
    def save_detection(self, frame_num, detections, fps, total_detections=0, unique_motos=0, detection_rate=0.0):
        ...
        for det in detections:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (datetime.now().isoformat(), frame_num, det['class'], det['class_name'], 
                  det['confidence'], bbox[0], bbox[1], bbox[2], bbox[3], det['area'],
                  fps, total_detections, unique_motos, detection_rate))
        
        conn.commit()
        conn.close()
    
    def save_detections(self, frame_num, detections, fps, total_detections=0, unique_motos=0, detection_rate=0.0):
        """Salva múltiplas detecções"""
        self.save_detection(frame_num, detections, fps, total_detections, unique_motos, detection_rate)
