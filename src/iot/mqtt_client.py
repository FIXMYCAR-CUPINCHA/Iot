...
class MQTTIoTClient:
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            print(f"📨 MQTT Recebido [{topic}]: {payload.get('sensor_id', payload.get('actuator_id', 'unknown'))}")
    ...
    def _process_sensor_data(self, data: Dict):
        ...
        try:
            ...
            print(f"⚠️ Falha HTTP para sensor {data['sensor_id']}, dados via MQTT apenas")
    ...
    def _process_actuator_data(self, data: Dict):
        ...
        try:
            ...
            response = requests.post(f"{self.api_url}/iot/actuator", 
                                  json=data, timeout=2)
            if response.status_code == 201:
                print(f"✅ Atuador {data['actuator_id']}: Dados processados via MQTT+HTTP")
        except requests.exceptions.RequestException:
            print(f"⚠️ Falha HTTP para atuador {data['actuator_id']}, dados via MQTT apenas")
