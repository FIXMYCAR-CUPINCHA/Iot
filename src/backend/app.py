...
def metrics():
    ...
    cursor.execute(
        "SELECT COUNT(DISTINCT class_name) FROM detections WHERE class_name IS NOT NULL"
    )
    ...
    cursor.execute(
        "SELECT fps FROM detections WHERE fps IS NOT NULL ORDER BY id DESC LIMIT 60"
    )
    ...
    cursor.execute(
        "SELECT AVG(detection_rate) FROM detections WHERE detection_rate > 0"
    )
